...
- [ ] [[id:C3E38617-59A2-4BB9-8E0D-04047CE5FB5B][Read up on =cycle-spacing=]]

*** Research [0/6]                                               :INACTIVE:
...
- [ ] [[https://github.com/oantolin/embark][Embark]]. Looks super cool. Maybe able to combine with Objed or Combobulate.
...
* Formatting
  ...
  I played around with using Eglot and the LSP for formatting and I might still do that but it was going to take some configuration to get it to not do annoying things with the formatting like always using actual tabs and defaulting to width 8 tabs. Both of those being 'issues' with how the LSP spec defines tabSize and what Emacs considers a tab and what it considers indentation.
...
:PROPERTIES:
:ID:       C3E38617-59A2-4BB9-8E0D-04047CE5FB5B
:END:

Emacs has two similar functions for deleting surrounding white space, =delete-horizontal-space= which deletes all whitespace surrounding the point on the current line and =delete-blank-lines= which deletes either blank lines above and below the current line if it is blank or deletes the current line if it is blank and the the lines surrounding lines are not. Below is a utility function to smartly combine the two.

Read up on =cycle-spacing= command. It looks like it might do much of what I'm doing here.
